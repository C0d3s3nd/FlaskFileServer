...
# pip install flask humanize pathlib2
...
def time_humanize(timestamp):
    mdate = str(datetime.utcfromtimestamp(timestamp))
    return  mdate[:mdate.rfind(":")]
...
class PathView(MethodView):
    ...
    def get(self, p=''):
        ...
        if os.path.isdir(path):
            ...
            res = make_response('Что-то пошло не так :(', 404)
        return res
    ...
    def put(self, p=''):
        if request.cookies.get('auth_cookie') == key:
            ...
            info = {} 
            info['status'] = 'error'
            info['msg'] = 'Ошибка аутентификации'
            ...
            if os.path.isdir(path):
                ...
                for file in files:
                    try:
                        ...
                        info['status'] = 'success'
                        info['msg'] = 'Файл сохранён'
            else:
                info['status'] = 'error'
                info['msg'] = 'Неверный оператор'
...
if __name__ == '__main__':
    ...
    root = os.path.normpath(os.getenv('FS_PATH', 'sh'))
    key = "Y29kZXI6eWZhYnVmbnQsdCduanllO3lqLHNrag==" # username:password :)
